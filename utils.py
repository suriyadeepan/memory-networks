...
def split_dataset(s, q, a, ratio = [0.7, 0.15, 0.15] ):
    ...
    testS, testQ, testA = s[lens[0]:lens[0]+lens[1]], q[lens[0]:lens[0]+lens[1]], a[lens[0]:lens[0]+lens[1]]
    validS, validQ, validA = s[-lens[-1]:], q[-lens[-1]:], a[-lens[-1]:]

    return (trainS, trainQ, trainA), (testS, testQ, testA), (validS, validQ, validA)
