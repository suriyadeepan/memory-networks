...
def zero_pad(stories, questions, answers, w2i):
    ...
    # sentence length for stories
    max_s_sent_len = max([ len(line.split(' ')) 
        for story in stories for line in story ])

    max_sent_len = max(max_q_sent_len, max_s_sent_len)

    idx_q = np.zeros([num_lines, max_sent_len + 1], dtype=np.int32)
    ...
    idx_s = np.zeros([num_lines, max_s_lines, max_sent_len + 1], 
            dtype=np.int32)
...
def process_data():
    ...
    # save them
    np.save('idx_q.npy', idx_q[:10000])
    np.save('idx_s.npy', idx_s[:10000])
    np.save('idx_a.npy', idx_a[:10000])
